...
async def login_via_ingress(identity) -> tuple[str, str]:
    ...
    Role assignment: the first HA user through the door becomes admin,
    matching the standalone container's first-run behaviour where whoever
    holds the bootstrap token becomes the owner. Later HA users get the
    configured default, which is 'readonly' — because Home Assistant's
    panel_admin only governs the sidebar entry, and silently promoting every
    household member to an EchoMuse admin would hand them the root shell
    proxy. An admin can raise them in the dashboard.
    """
    loop = asyncio.get_event_loop()
